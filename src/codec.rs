...
mod tests {
    ...
    /// the starved scratch is included because that is the only regime where the
    /// widened arm is reachable at all. This reaches `step` directly, so it holds
    /// only for the backend this workspace resolves: the validation arms see the
    /// public API, where the property is not observable, and a C-zlib divergence
    /// in it would go unguarded here.
    #[test]
    fn output_cannot_precede_input_at_a_stream_start() {
        ...
        for peer in 8..=15u8 {
            ...
            for width in [1usize, 2, 4096] {
                ...
                assert!(opened.produced > 0 || opened.remaining.is_some(), "the run-up moved");
